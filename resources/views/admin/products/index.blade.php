        ...
        <div class="page-header-content pt-4">
            <div class="row align-items-center justify-content-between">
                <div class="col-auto mt-4">
                    ...
                    <div class="page-header-subtitle">Here are all your Products
                    </div>
                </div>
    ...
    <div class="card mb-4">
        <div class="card-header"><div class="" style="display: flex;justify-content:space-between">
            ...
            <div class="badge bg-primary text-white rounded-pill" x-data='xlien'>
                <a x-on:click='redirect( "{{ Route('products.create') }}" )' class="text-white pointer">Add</a> 
            </div>
        </div></div>
        <div class="card-body" x-data='xdatas("products", 5)'>
            <div class="dataTable-wrapper dataTable-loading no-footer sortable searchable fixed-columns">
                <div class="dataTable-container">
                    <table class="dataTable-table">
                        ...
                        <tbody>
                            <template x-for='product in datas'>
                                <tr>
                                    <td x-text='ucword(product.name)'></td>
                                    <td x-text='ucword(product.shop.intitule)'></td>
                                    <td x-text='product.price'></td>
                                    <td x-text='product.special_price'></td>
                                    <td x-text='ucword(product.category.name)'></td>
                                    <td>
                                        <div x-on:click='redirect(product.edit_url)' class="pointer badge bg-warning text-white rounded-pill">
                                            edit</div>
                                        <div x-on:click='redirect(product.attribute_url)' class="pointer badge bg-primary text-white rounded-pill">attributes</div>
                                        
                                        <template x-if='product.state == "enabled" || product.state == "init"'>
                                            <div x-on:click="state(product.id, 'disabled')" style="background-color: green;" class="pointer badge text-white rounded-pill">activated</div>
                                        </template>
                                        
                                        <template x-if='product.state == "disabled"'>
...
<script>
    function state(id, state) {
        axios.get('/api/products/state/'+ id +'/'+ state)
        .then(response => {
            location.reload();
        })
    }
